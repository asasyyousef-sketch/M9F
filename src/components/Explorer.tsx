...
import { speakClient, preloadTTS, preloadImage, fetchGradioAudioBlob, fetchAudioBlob } from "./Modals";
...
export const Explorer: React.FC<ExplorerProps> = React.memo(({
      ...
      if (downloadIncludeFront && card.frontText && card.frontText.trim()) {
        if (card.frontAudioUrl) {
          queue.push({
            text: card.frontText,
            lang: fLang,
            label: `تخزين صوت الوجه الأمامي (ملف مخصص) - "${card.frontText}"`,
          ...
          if (downloadVoiceTarget === "primary" || downloadVoiceTarget === "both") {
            queue.push({
              text: card.frontText,
              lang: fLang,
              label: `تخزين صوت الوجه الأمامي - الصوت الرئيسي (${primaryVoiceF}) - "${card.frontText}"`,
              voice: primaryVoiceF,
              target: "primary",
          ...
          if (downloadVoiceTarget === "secondary" || downloadVoiceTarget === "both") {
            queue.push({
              text: card.frontText,
              lang: fLang,
              label: `تخزين صوت الوجه الأمامي - الصوت الثانوي (${secondaryVoiceF}) - "${card.frontText}"`,
      ...
      if (downloadIncludeBack && card.backText && card.backText.trim()) {
        if (card.backAudioUrl) {
          queue.push({
            text: card.backText,
            lang: bLang,
            label: `تخزين صوت الوجه الخلفي (ملف مخصص) - "${card.backText}"`,
          ...
          if (downloadVoiceTarget === "primary" || downloadVoiceTarget === "both") {
            queue.push({
              text: card.backText,
              lang: bLang,
              label: `تخزين صوت الوجه الخلفي - الصوت الرئيسي (${primaryVoiceB}) - "${card.backText}"`,
              voice: primaryVoiceB,
              target: "primary",
          ...
          if (downloadVoiceTarget === "secondary" || downloadVoiceTarget === "both") {
            queue.push({
              text: card.backText,
              lang: bLang,
              label: `تخزين صوت الوجه الخلفي - الصوت الثانوي (${secondaryVoiceB}) - "${card.backText}"`,
      ...
      if (downloadIncludePlural && card.isPluralMode && card.pluralText && card.pluralText.trim()) {
        ...
        if (downloadVoiceTarget === "primary" || downloadVoiceTarget === "both") {
          queue.push({
            ...
            lang: pLang,
            label: `تخزين صوت صيغة الجمع - الصوت الرئيسي (${primaryVoiceP}) - "${card.pluralText}"`,
            voice: primaryVoiceP,
            target: "primary",
        ...
        if (downloadVoiceTarget === "secondary" || downloadVoiceTarget === "both") {
          queue.push({
            text: card.pluralText,
            lang: pLang,
            label: `تخزين صوت صيغة الجمع - الصوت الثانوي (${secondaryVoiceP}) - "${card.pluralText}"`,
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      try {
        if (item.customUrl) {
          ...
          // Preload & Cache in browser offline storage (Cache Storage + Memory)
  ...
  const handleDownloadMp3Single = React.useCallback(async (text: string, lang: string, filename: string, voiceTarget: "primary" | "secondary" = "primary") => {
    try {
      ...
      const isGradioVoice =
        voice.startsWith("gradio:") ||
        voice.startsWith("gradio_") ||
        ["ryan", "serena", "vivian", "aiden", "eric", "dylan", "uncle_fu", "ono_anna", "sohee"].includes(voice.toLowerCase());
      ...
      if (isGradioVoice) {
        ...
        const blob = await fetchGradioAudioBlob(text, cleanVoice, lang, gradioUrl);
        if (blob) {
          const a = document.createElement("a");
          a.href = URL.createObjectURL(blob);
          a.download = `${filename || "audio"}_${voiceTarget}.wav`;
        ...
        <div className="flex flex-col gap-2 w-full z-10 relative text-right">
          ...
          <div className="flex flex-wrap items-center gap-1.5 md:gap-2 pt-0.5">
            ...
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 md:px-3 md:py-1 text-[10px] md:text-[11px] font-bold rounded-lg border ${
              ...
              <span>{activeFolderId ? getRecursiveCardCount(activeFolderId) : (searchQuery ? displayedCards.length : cards.length)} بطاقات</span>
            </span>
          </div>
        </div>
          ...
          <div className="bg-white border border-slate-100 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-5 text-right relative overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-black text-slate-800 leading-tight">
                  تنزيل وحفظ الصوتيات (ZIP وأوفلاين)
                </h3>
                <p className="text-xs text-slate-500 font-semibold mt-0.5">
                  حفظ مقاطع النطق كملف مضغوط ZIP على جهازك أو تخزينها بالمتصفح للعمل بدون إنترنت
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-1">
              <button
                ...
                className="w-full py-3.5 px-4 rounded-2xl font-black text-sm text-white shadow-md hover:brightness-110 active:scale-98 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                style={{ backgroundColor: folderColor }}
              >
                {isDownloadingAudio ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" />
                    <span>جاري التخزين بالمتصفح ({audioProgress.current}/{audioProgress.total})...</span>
                  </>
                ) : (
                  <>
                    <DownloadCloud className="w-5 h-5" />
                    <span>تنزيل وتخزين كافة الصوتيات في المتصفح أوفلاين ⚡</span>
                  </>
                )}
              </button>

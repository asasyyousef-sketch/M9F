...
export const ReviewSession: React.FC<ReviewSessionProps> = React.memo(({
  ...
  useEffect(() => {
    localStorage.setItem("settings_preload_audio_review", String(isAudioPreloadEnabled));
  }, [isAudioPreloadEnabled]);
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("focus", handleSyncPreload);
      window.removeEventListener("storage", handleSyncPreload);
    };
  }, []);
  ...
  const playPronunciationSingle = (text: string, lang: string, onEnd?: () => void, bypassMute = false, voiceOverride?: string) => {
    ...
    if (isGradioVoice) {
      ...
      const cleanVoiceLang = (lang || "de").toLowerCase().split("-")[0].split("_")[0];
      const isSecondary = effectiveVoice === localStorage.getItem(`settings_secondary_piper_model_${cleanVoiceLang}`) ||
                          effectiveVoice === localStorage.getItem("settings_secondary_piper_model") ||
                          reviewVoiceTarget === "secondary";
      const customGradioLang = isSecondary
        ? localStorage.getItem(`settings_secondary_gradio_lang_${cleanVoiceLang}`)
        : localStorage.getItem(`settings_primary_gradio_lang_${cleanVoiceLang}`);
      const effectiveGradioLang = customGradioLang || lang;
      const cacheKey = `${cleanText}_${effectiveGradioLang}_gradio:${gradioVoiceName}`;
      ...
      playGradioClientAudio(cleanText, gradioVoiceName, effectiveGradioLang, gradioUrl).then((result) => {
        if (!result.ok) {
          console.warn("Gradio TTS playback failed in review:", result.error);
        }
        if (onEnd) onEnd();
      }).catch(() => {
        if (onEnd) onEnd();
  ...
  useEffect(() => {
    ...
    const preloadCardAssets = async (card: Flashcard) => {
      ...
      // Preload TTS Audio only when enabled in settings
      if (isAudioPreloadEnabled) {
        // Preload front text
        if (card.frontText) {
  ...
  }, [currentIndex, sessionCards, method, isAudioPreloadEnabled]);
        ...
        <AnimatePresence mode="wait">
          ...
          {isCompleted ? (
            ...
            >
              ...
              {method !== "match" && method !== "article" && method !== "write" && method !== "listen" && method !== "repetition" && currentCard && (
              ...
              {method === "repetition" && currentCard && (
                <div className="w-full flex flex-col items-center">
                  <RepetitionSessionView
                    card={currentCard}
                    currentIndex={currentIndex}
                    totalCards={sessionCards.length}
                    onNext={handleNextCard}
                    onPrev={handlePrevCard}
                    onKnow={() => handleClassicKnow(true)}
                    onRepeat={() => handleClassicKnow(false)}
            ...
            <div className="p-5 overflow-y-auto space-y-5 text-xs text-slate-600 max-h-[60vh]">
              ...
              {activeSettingsTab === "sound" && (
                <div className="space-y-4 animate-fadeIn">
                  ...
                  <div className="flex items-center justify-between py-1 border-b border-slate-50">
                    <div className="flex flex-col gap-0.5 text-right">
                      <span className="text-xs font-bold text-slate-700">التحميل المسبق للأصوات في المراجعة</span>
                      <span className="text-[10px] text-slate-400">تنزيل وتجهيز نطق البطاقات مسبقاً في الخلفية لتسريع تشغيل الصوت</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => setIsAudioPreloadEnabled(!isAudioPreloadEnabled)}
                      className={`w-11 h-6 rounded-full transition-colors relative cursor-pointer shrink-0 ${isAudioPreloadEnabled ? 'bg-emerald-500' : 'bg-slate-200'}`}
                    >
                      <span className={`absolute top-0.5 left-0.5 bg-white w-5 h-5 rounded-full transition-transform duration-200 ${isAudioPreloadEnabled ? 'translate-x-5' : 'translate-x-0'}`} />
                    </button>
                  </div>

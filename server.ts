...
`;
  ...
  // Fast Shadowing Speech-to-Text Endpoint (Works across all browsers, iframes, Firefox, Chrome, Safari)
  app.post("/api/shadowing/transcribe", async (req, res) => {
    try {
      const { audioBase64, mimeType = "audio/webm", language = "de", targetSentence = "" } = req.body;
      if (!audioBase64) {
        return res.status(400).json({ error: "لا يوجد بيانات صوتية مرسلة" });
      }
      ...
      const headerKey = (req.headers["x-gemini-key"] as string) || "";
      const effectiveGeminiKey =
        (req.body.customApiKey && req.body.customApiKey.trim()) ||
      ...
      const langName = langMap[language.slice(0, 2).toLowerCase()] || language;

      const promptText = `Act as an ultra-fast, literal speech-to-text transcriber for shadowing practice.
Language of speech: ${langName}.
${targetSentence ? `The sentence the user was practicing: "${targetSentence}"` : ""}

Transcribe ONLY what you hear in the audio. If the user spoke words, output those exact words with spaces. 
Do not output quotes, explanations, markdown, or timestamps. If the audio is silent or unintelligible noise, output empty string.`;
      ...
      console.error("Shadowing transcription error:", err);
      return res.status(500).json({ error: err?.message || "فشل تفريغ الصوت" });
    }
  });
...
${JSON.stringify(simplifiedCards, null, 2)}`;
    ...
    try {
      ...
      if (supabase) {
        ...
        while (true) {
          ...
          allDecks.push(...chunk);
          if (chunk.length < deckStep) break;
          deckFrom += deckStep;
        }
        ...
        while (true) {
          ...
          if (!chunk || chunk.length === 0) break;
          allCards.push(...chunk);
          if (chunk.length < cardStep) break;
          cardFrom += cardStep;
        }
        ...
        if (decksErr || cardsErr) {
          ...
          // If the tables exist but are completely empty, seed them automatically
          if (decks.length === 0 && cards.length === 0) {
            ...
            // 3. Insert cards in batches of 500
            ...
            for (let i = 0; i < cardsToInsert.length; i += 500) {
              const batch = cardsToInsert.slice(i, i + 500);
        ...
        (async () => {
          try {
            ...
            if (allDbDecks.length > 0 && activeDeckIds.size > 0) {
              ...
              if (decksToDelete.length > 0) {
                ...
                for (let i = 0; i < decksToDelete.length; i += 500) {
                  ...
                  await supabase.from('decks').delete().in('id', chunk);
                }
              }
            }
            ...
            if (decksNoParent.length > 0) {
              for (let i = 0; i < decksNoParent.length; i += 500) {
                const { error: dErr1 } = await supabase.from('decks').upsert(decksNoParent.slice(i, i + 500));
            ...
            if (decksWithParent.length > 0) {
              for (let i = 0; i < decksWithParent.length; i += 500) {
                const { error: dErr2 } = await supabase.from('decks').upsert(decksWithParent.slice(i, i + 500));
                if (dErr2) throw dErr2;
              }
            }

            // 4. Upsert all active cards in batches
            ...
            if (cardsToInsert.length > 0) {
              for (let i = 0; i < cardsToInsert.length; i += 500) {
                const { error: cErr } = await supabase.from('cards').upsert(cardsToInsert.slice(i, i + 500));
                if (cErr) throw cErr;
              }
            }
  ...
  app.post("/api/sync/push", async (req, res) => {
    try {
      ...
      console.log(`[Manual Sync Push] Direct upload of ${folders.length} folders and ${cards.length} cards in batches...`);

      // Get existing decks/cards using chunking
      let allDbDecks: any[] = [];
      let dFrom = 0;
      while (true) {
        const { data: chunk } = await supabase.from('decks').select('id').range(dFrom, dFrom + 999);
      ...
      // 1. Pass 1: Upsert decks with parentId = null in batches
      const decksNoParent = folders.map((f: any, index: number) => ({
        id: f.id,
        parentId: null,
        name: f.name,
      ...
      // 2. Pass 2: Upsert decks with parentId resolved in batches
      const decksWithParent = folders.map((f: any, index: number) => ({
        id: f.id,
        parentId: f.parentId || null,
        name: f.name,
  ...
  app.post("/api/sync/pull", async (req, res) => {
    try {
      ...
      console.log("[Manual Sync Pull] Fetching folders and cards from Supabase with chunking...");
  ...
  const handleGetTtsCatalog = (req: express.Request, res: express.Response) => {
    ...
    let installedMap: Record<string, { sizeMb: string; meta?: any }> = {};
    if (fs.existsSync(modelsDir)) {
      ...
      files.filter(f => f.endsWith(".onnx")).forEach(file => {
        ...
        let meta: any = null;
        const metaPath = path.join(modelsDir, `${id}.meta.json`);
    ...
    const result: any[] = catalog.map(item => ({
      ...item,
    ...
    Object.keys(installedMap).forEach(id => {
      if (!result.some(r => r.id === id)) {
        const meta = installedMap[id].meta;
        const parts = id.split("-");
        const langCode = parts[0] || "custom";
        const voiceName = parts[1] || id;
        const quality = parts[2] || "custom";
        const langShort = langCode.includes("_") ? langCode.split("_")[0] : (langCode === "custom" ? "custom" : langCode);

        const urlOnnx = meta?.urlOnnx || `https://huggingface.co/rhasspy/piper-voices/resolve/main/${langShort}/${langCode}/${voiceName}/${quality}/${id}.onnx`;
        const urlJson = meta?.urlJson || `https://huggingface.co/rhasspy/piper-voices/resolve/main/${langShort}/${langCode}/${voiceName}/${quality}/${id}.onnx.json`;

        let flag = "🎯";
        if (langShort === "de") flag = "🇩🇪";
        else if (langShort === "ar") flag = "🇯🇴";
        else if (langShort === "en") flag = "🇺🇸";
        else if (langShort === "fr") flag = "🇫🇷";
        else if (langShort === "es") flag = "🇪🇸";

        result.push({
          id,
          name: meta?.name || id,
          lang: langShort,
          langName: langShort === "custom" ? "صوت مخصص (Custom)" : (meta?.name || id),
          flag,
          quality: "Custom Neural",
          sample: meta?.sample || "تجربة الصوت المخصص",
          urlOnnx,
          urlJson,
          isDownloaded: true,
  ...
  app.get("/api/tts/catalog", handleGetTtsCatalog);
  app.get("/api/tts/models/catalog", handleGetTtsCatalog);
  ...
  const ttsModelDownloadProgress: Record<string, {
    ...
    step: string;
    files?: {
      onnx: { name: string; status: "pending" | "downloading" | "completed" | "error"; sizeMb?: string; sizeBytes?: number; error?: string };
      json: { name: string; status: "pending" | "downloading" | "completed" | "error"; sizeKb?: string; sizeBytes?: number; error?: string };
    };
  }> = {};
  ...
  app.get("/api/tts/models/download-progress", (req, res) => {
    ...
    const prog = ttsModelDownloadProgress[modelId] || {
      ...
      files: {
        onnx: { name: `${modelId}.onnx`, status: "pending" },
        json: { name: `${modelId}.onnx.json`, status: "pending" }
      }
    };
  ...
  app.post("/api/tts/models/download", express.json(), async (req, res) => {
    ...
    try {
      ...
      ttsModelDownloadProgress[modelId] = {
        ...
        speed: "0.0 MB/s",
        status: "downloading",
        step: "جاري بدء الاتصال وجلب ملف النموذج الأساسي (.onnx)...",
        files: {
          onnx: { name: `${modelId}.onnx`, status: "downloading" },
          json: { name: `${modelId}.onnx.json`, status: "pending" }
        }
      };
      ...
      const downloadStream = async (url: string, destPath: string, isMainOnnx: boolean, sizeMbHint?: string) => {
        ...
        if (!fetchRes.ok) {
          throw new Error(`HTTP ${fetchRes.status}: ${fetchRes.statusText} عند محاولة تحميل ${path.basename(destPath)}`);
        }
              ...
              if (isMainOnnx) {
                ...
                if (timeDiff >= 0.25) {
                  ...
                  if (speedMb >= 1) {
                    ...
                    currentSpeed = `${Math.round(bytesDiff / (1024 * timeDiff))} KB/s`;
                  }
                  lastTime = now;
                  lastLoaded = loaded;
                }

                const percent = total > 0 ? Math.min(90, Math.round((loaded / total) * 90)) : 10;
                ...
                ttsModelDownloadProgress[modelId] = {
                  ...
                  speed: currentSpeed,
                  status: "downloading",
                  step: `[1/2] تنزيل ملف النموذج: ${loadedMb} / ${totalMb} (${percent}%) - ⚡ ${currentSpeed}`,
                  files: {
                    onnx: { name: `${modelId}.onnx`, status: "downloading", sizeMb: loadedMb, sizeBytes: loaded },
                    json: { name: `${modelId}.onnx.json`, status: "pending" }
                  }
                };
              }
      ...
      try {
        ...
        throw new Error(`ملف النموذج الأساسي (.onnx): ${onnxErr.message}`);
      }

      if (!fs.existsSync(onnxPath) || fs.statSync(onnxPath).size < 1024) {
        throw new Error("ملف النموذج الأساسي (.onnx) غير صالح أو حجمه فارغ.");
      }

      const onnxStats = fs.statSync(onnxPath);
      const onnxSizeMb = (onnxStats.size / (1024 * 1024)).toFixed(1) + " MB";
      ...
      if (finalJsonUrl) {
        if (ttsModelDownloadProgress[modelId]) {
          ttsModelDownloadProgress[modelId].percent = 92;
          ttsModelDownloadProgress[modelId].step = "[2/2] جاري تنزيل والتحقق من ملف الإعدادات (.onnx.json)...";
          if (ttsModelDownloadProgress[modelId].files) {
            ttsModelDownloadProgress[modelId].files!.onnx = {
              name: `${modelId}.onnx`,
              status: "completed",
              sizeMb: onnxSizeMb,
              sizeBytes: onnxStats.size
            };
            ttsModelDownloadProgress[modelId].files!.json = {
              name: `${modelId}.onnx.json`,
              status: "downloading"
            };
          }
        }
        ...
        try {
          ...
          throw new Error(`ملف التوصيف والإعدادات (.onnx.json): ${jsonErr.message}`);
        }

        if (fs.existsSync(jsonPath) && fs.statSync(jsonPath).size > 10) {
          const jsonStats = fs.statSync(jsonPath);
          jsonSizeBytes = jsonStats.size;
          jsonSizeKb = (jsonStats.size / 1024).toFixed(1) + " KB";

          // Verify it is valid JSON
          try {
            JSON.parse(fs.readFileSync(jsonPath, "utf-8"));
          } catch (parseErr) {
            console.warn("JSON config file parsing warning:", parseErr);
          }
        }
      }

      // Total size
      const totalBytes = onnxStats.size + jsonSizeBytes;
      const totalSizeMb = (totalBytes / (1024 * 1024)).toFixed(1) + " MB";

      // Save metadata for custom or catalog models
      const metaPath = path.join(modelsDir, `${modelId}.meta.json`);
      try {
        fs.writeFileSync(metaPath, JSON.stringify({
          id: modelId,
          name: req.body.name || modelId,
          urlOnnx: finalOnnxUrl,
          urlJson: finalJsonUrl,
          sizeMb: onnxSizeMb,
          jsonSizeKb,
          totalSizeMb,
      ...
      return res.json({
        ...
        totalSizeMb,
        files: {
          onnx: { name: `${modelId}.onnx`, status: "completed", sizeMb: onnxSizeMb },
          json: { name: `${modelId}.onnx.json`, status: "completed", sizeKb: jsonSizeKb }
        },
        message: `تم تنزيل ملف النموذج (${onnxSizeMb}) وملف الإعدادات (${jsonSizeKb}) بنجاح.`
      });
    } catch (err: any) {
      ...
      const errorMessage = err?.message || "فشل التنزيل";
      ...
      ttsModelDownloadProgress[modelId] = {
        ...
        speed: "0.0 MB/s",
        status: "error",
        step: `فشل التنزيل: ${errorMessage}`,
        files: {
          onnx: {
            name: `${modelId}.onnx`,
            status: errorMessage.includes(".onnx") ? "error" : "completed",
            error: errorMessage.includes(".onnx") ? errorMessage : undefined
          },
          json: {
            name: `${modelId}.onnx.json`,
            status: errorMessage.includes(".json") ? "error" : "pending",
            error: errorMessage.includes(".json") ? errorMessage : undefined
          }
        }
      };
  ...
  const handleDeleteModelEndpoint = (modelId: string, res: express.Response) => {
  ...
  app.delete("/api/tts/models/:modelId", (req, res) => {
    handleDeleteModelEndpoint(req.params.modelId, res);
  });

  app.post("/api/tts/models/delete", express.json(), (req, res) => {
    handleDeleteModelEndpoint(req.body.modelId, res);
  });
  ...
  // Helper for Gradio Qwen3-TTS / Local TTS Server
  async function generateGradioTtsAudio(
    text: string,
    voice: string = "ryan",
    lang: string = "german",
    ...
    let cleanLang = (lang || "german").toLowerCase().trim();
    const langMap: Record<string, string> = {
      de: "german",
      en: "english",
      ar: "arabic",
    ...
    if (langMap[cleanLang]) {
      cleanLang = langMap[cleanLang];
    }

    // Dynamic timeout calculation:
    // Base 180s (3 minutes) + 500ms per character, capped up to 10 minutes (600,000ms)
    // This allows Qwen3-TTS ample time to generate long texts/paragraphs without timing out prematurely
    const dynamicTimeoutMs = Math.max(180000, Math.min(600000, 180000 + text.length * 500));

    // Direct endpoint as requested: http://.../gradio_api/call/generate
    const generateEndpoint = `${normalizedUrl}/gradio_api/call/generate`;
    ...
    try {
      ...
      if (postRes.ok) {
        ...
        if (eventId) {
          sseAttempted = true;
          // 2. GET request for SSE stream: /gradio_api/call/generate/{event_id}
          const streamRes = await fetch(`${generateEndpoint}/${eventId}`, {
          ...
          if (streamRes.ok) {
            ...
            // Process SSE lines from bottom or loop through all messages to find the final complete audio output
            ...
            for (let i = lines.length - 1; i >= 0; i--) {
              ...
              if (line.startsWith("data:")) {
                try {
                  ...
                  // Check if it's the final output payload (array containing audio object or path)
                  const candidateArray = Array.isArray(dataJson) ? dataJson : [dataJson];
                  for (const candidate of candidateArray) {
                    if (candidate && typeof candidate === "object") {
                      if (candidate.url) {
                        foundAudioUrl = candidate.url;
                        break;
                      } else if (candidate.path) {
                        foundAudioUrl = `${normalizedUrl}/file=${candidate.path}`;
                        break;
                      }
                      ...
                      foundAudioUrl = candidate.startsWith("http") ? candidate : `${normalizedUrl}/file=${candidate}`;
                      break;
                    }
                  }
                  if (foundAudioUrl) break;
                } catch (parseErr) {}
              }
            }

            if (foundAudioUrl) {
              const targetUrl = foundAudioUrl.startsWith("http")
                ? foundAudioUrl
                : `${normalizedUrl}${foundAudioUrl.startsWith("/") ? "" : "/"}${foundAudioUrl}`;

              const finalAudioRes = await fetch(targetUrl, {
                signal: AbortSignal.timeout(60000)
              });

              if (finalAudioRes.ok) {
                const arrayBuffer = await finalAudioRes.arrayBuffer();
                return {
                  buffer: Buffer.from(arrayBuffer),
                  mimeType: finalAudioRes.headers.get("content-type") || "audio/wav"
                };
              }
            }
          }
        }
      } else if (postRes.status === 404 || postRes.status === 405) {
        // Only if endpoint is not supported, allow fallback to legacy /run/predict
        sseAttempted = false;
      }
    } catch (sseErr: any) {
      console.warn("Gradio generate SSE error on server proxy:", sseErr);
      lastError = sseErr;
    }

    // Fallback to /run/predict ONLY if /gradio_api/call/generate was not supported on the server
    // NEVER retry if SSE was active and timed out, to protect host machine CPU/GPU from request cascades
    if (!sseAttempted) {
      try {
        ...
        for (const payload of payloadVariations) {
          try {
            const postRes = await fetch(fallbackEndpoint, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            ...
            if (postRes.ok) {
              const resData = (await postRes.json()) as any;
              const dataArray = resData?.data;
              if (Array.isArray(dataArray) && dataArray.length > 0) {
                const firstItem = dataArray[0];
                if (typeof firstItem === "string" && firstItem.startsWith("data:audio")) {
                ...
                const audioUrl =
                  firstItem?.url ||
                  (firstItem?.name ? `${normalizedUrl}/file=${firstItem.name}` : null);
                if (audioUrl) {
                  const targetUrl = audioUrl.startsWith("http")
                    ? audioUrl
                    : `${normalizedUrl}${audioUrl.startsWith("/") ? "" : "/"}${audioUrl}`;
                  const finalAudioRes = await fetch(targetUrl, {
                    signal: AbortSignal.timeout(60000)
                  });
                  if (finalAudioRes.ok) {
    ...
    throw new Error(
      `تعذر توليد الصوت من سيرفر Gradio (${serverUrl}): ${lastError?.message || "لم يعطِ السيرفر استجابة صوتية صحيحة"}`
    );
  }

  // Endpoint: Test connection to local Gradio Qwen3-TTS server
  app.all("/api/tts/gradio/test", express.json(), async (req, res) => {
    const serverUrl = (req.query.serverUrl || req.body?.serverUrl || "http://192.168.0.159:7860") as string;
    const normalizedUrl = serverUrl.replace(/\/+$/, "");

    const startTime = Date.now();
    try {
      // 1. Ping /config or /info or /
      let isReachable = false;
      let appTitle = "Gradio TTS Server";
      let details: any = {};
      ...
      for (const ep of testEndpoints) {
        try {
          const pingRes = await fetch(ep, { signal: AbortSignal.timeout(4000) });
          if (pingRes.ok) {
            isReachable = true;
            try {
              const json = await pingRes.json();
              if (json?.app_title || json?.title) appTitle = json.app_title || json.title;
              details = { endpointTested: ep, status: pingRes.status };
            } catch (e) {
              details = { endpointTested: ep, status: pingRes.status, isHtml: true };
            }
            break;
          }
        } catch (pingErr) {}
      ...
      if (!isReachable) {
        return res.status(503).json({
          ...
          serverUrl,
          latencyMs,
          error: `تعذر الوصول إلى سيرفر Gradio على العنوان ${serverUrl}`,
          suggestedSolution: "تأكد أن لابتوبك متصل بنفس شبكة الواي فاي، وأن سيرفر Gradio يعمل على المنفذ 7860 (أو فعّل خيار share=True إذا كنت خارج الشبكة المحلية)."
        });
      }
      ...
      return res.json({
        ...
        details,
        availableVoices: [
          { id: "ryan", name: "Ryan (ذكوري طبيعي - إنجليزي/ألماني)", lang: "en/de", gender: "male" },
          { id: "serena", name: "Serena (أنثوي ناعم وواضح)", lang: "en/de", gender: "female" },
          { id: "vivian", name: "Vivian (أنثوي رنان)", lang: "en/de", gender: "female" },
          { id: "aiden", name: "Aiden (ذكوري شاب وحيوي)", lang: "en/de", gender: "male" },
          { id: "eric", name: "Eric (ذكوري هادئ ورزين)", lang: "en/de", gender: "male" },
          { id: "dylan", name: "Dylan (ذكوري عصري)", lang: "en/de", gender: "male" },
          { id: "uncle_fu", name: "Uncle Fu (ذكوري صيني)", lang: "zh", gender: "male" },
          { id: "ono_anna", name: "Ono Anna (أنثوي ياباني)", lang: "ja", gender: "female" },
          { id: "sohee", name: "Sohee (أنثوي كوري)", lang: "ko", gender: "female" }
        ],
        availableLanguages: [
          "german", "english", "auto", "chinese", "french", "italian", "japanese", "korean", "portuguese", "russian", "spanish", "arabic"
        ]
      });
    } catch (err: any) {
      return res.status(500).json({
        ...
        error: err?.message || "خطأ أثناء فحص اتصال سيرفر Gradio"
      });
    }
  });
  ...
  app.post("/api/tts/gradio/generate", express.json(), async (req, res) => {
    ...
    try {
      ...
      return res.status(502).json({
        error: err?.message || "فشل توليد الصوت من سيرفر Gradio",
        serverUrl
      });
    }
  });
  ...
  app.get("/api/tts", async (req, res) => {
    ...
    try {
      ...
      // 0. Gradio TTS Engine (Qwen3-TTS / Local Gradio Server)
      const reqVoiceLower = (requestedVoice || "").toLowerCase();
      const isGradioRequested =
        reqVoiceLower.startsWith("gradio:") ||
      ...
      if (isGradioRequested) {
        ...
        try {
          const gradioResult = await generateGradioTtsAudio(
            cleanText,
            gradioVoiceName,
            lang,
            gradioServerUrl,
          ...
          return res.status(502).json({
            error: `تعذر توليد الصوت من سيرفر Gradio الخارجي (Qwen3-TTS) على ${gradioServerUrl}: ${gradioErr?.message || "تأكد من تشغيل السيرفر ومن العنوان"}`,
            provider: "gradio",
            serverUrl: gradioServerUrl,
            voice: gradioVoiceName
          });
        }
      }
...
function sanitizeCardArticleAndPlural(card: any): any {
  ...
  app.post("/api/ai/correct", express.json(), async (req, res) => {
    try {
      const { text, targetLanguage, targetLevel, chatHistory, customApiKey, geminiApiKey, groqApiKey: bodyGroqApiKey, sendMode, selectedModel } = req.body;
...
4. checklist: مصفوفة متسلسلة تضمن التغطية الشاملة والكاملة (100% Coverage) لكل المتغيرات والشروط المدخلة في سياق التمرين.
  ...
  // Dedicated AI Review Mode Chat & Flashcard Corrector Endpoint
  app.post("/api/ai/review-chat", express.json(), async (req, res) => {
    try {
      const {
        card,
        previousCards = [],
        ...
        useStructuredTemplate = true, // Boolean toggle for mandatory sections
        teachingTone = "tutor", // 'tutor' | 'corrector' | 'linguist' | 'friendly'
        customApiKey,
        geminiApiKey,
        groqApiKey: bodyGroqApiKey
      } = req.body;
      ...
      const cardFront = card?.frontText || "";
      const cardBack = card?.backText || "";
      const cardHint = card?.translationHint || "";
      const cardArticle = card?.correctArticle || "";
      const cardPlural = card?.pluralText || "";
      const frontLang = card?.frontLang || folderInfo?.targetLanguage || "de";
      const backLang = card?.backLang || folderInfo?.sourceLanguage || "ar";
      const folderTitle = folderInfo?.name || "مجموعة تعليمية";
      const folderDesc = folderInfo?.description || "بطاقات مراجعة ومفردات";

      let lengthDirective = "";
      if (responseLength === "concise") {
        lengthDirective = `⚡ **وضع الإيجاز والتركيز الذكي (Concise Mode - أولوية مطلقة)**:
- 🚨 **قاعدة كسر نمط وسياق المحادثة (History Bias Override)**: حتى لو كانت الردود السابقة في هذه المحادثة طويلة أو مفصلة أو مقسمة لعناوين، **تجاهل طولها تماماً** ولا تحاكيها!
- **الهدف**: الإيجاز الذكي والمباشر. أعطِ زبدة الجواب وخلاصة المعنى أو القاعدة مباشرة دون لف أو دوران أو حشو.
- 🛑 **المحظورات**: تجنب تماماً المقدمات الإنشائية والترحيبية والخواتم، وتجنب الجداول الطويلة أو سرد تفاصيل وتصريفات وقواعد إضافية لم يطلبها المستخدم.
- **المرونة**: إذا كان السؤال بطبيعته بسيطاً، أجب في سطرين أو ثلاث. وإذا كان السؤال يتطلب شرحاً لتوضيح الفكرة، فاشرحه بأبسط وأقصر عبارة كافية ووافية دون بتر المعنى.`;
      } else if (responseLength === "auto") {
        lengthDirective = `🎯 **الوضع التلقائي الذكي (Auto Contextual Length - حسب طبيعة السؤال)**:
- **تحليل طبيعة السؤال لتحديد عمق وطول الرد بدقة**:
  * **إذا كان سؤال المستخدم بسيطاً أو مباشراً** (مثل: ما معنى الكلمة؟ ما أداتها؟ ما جمعها؟ ما إعرابها السريع؟ ترجم هذه العبارة): قدّم إجابة مباشرة وموجزة وسريعة في سطر إلى سطرين أو نقاط مكثفة دون استطراد أو حشو غير مطلوب.
  * **إذا كان سؤال المستخدم تحليلياً أو يطلب مقارنة أو شرحاً متعمقاً** (مثل: ما الفرق بين كلمتين؟ اشرح قاعدة نحوية كاملة، أعطني تصريفات وأمثلة متعددة، وضح سياقات الاستخدام): قدّم شرحاً وافياً وغنياً بالأمثلة والجداول التوضيحية لتغطية السؤال بشكل كامل.
  * حدد طول الرد تلقائياً بناءً على ما يحتاجه السؤال تحديداً لإيصال المعلومة بأفضل كفاءة.`;
      } else if (responseLength === "detailed") {
        lengthDirective = `📚 **وضع الشرح الشامل والمفصل (Detailed Mode - أولوية مطلقة)**:
- 🚨 **قاعدة كسر نمط المحادثة**: حتى لو كانت الردود السابقة في المحادثة قصيرة، يرجى تقديم شرح ثري ومتعمق وشامل يغطي القواعد النحوية، تصريفات الأفعال، الفروقات الدلالية، وأمثلة حية وسياقية وجداول توضيحية.`;
      } else {
        lengthDirective = `⚖️ **الوضع المتوازن (Balanced Mode)**:
- إجابة متوازنة وواضحة تركز على الفائدة اللغوية المباشرة مع مثال أو مثالين دون إطالة مفرطة أو حشو.`;
      }

      // Format surrounding context (for background context ONLY)
      const prevContext = Array.isArray(previousCards) && previousCards.length > 0
        ? previousCards.map((c: any, i: number) => `  - [${c.correctArticle ? c.correctArticle + ' ' : ''}${c.frontText}] (${c.backText})`).join('\n')
        : "  (لا توجد)";

      const nextContext = Array.isArray(nextCards) && nextCards.length > 0
        ? nextCards.map((c: any, i: number) => `  - [${c.correctArticle ? c.correctArticle + ' ' : ''}${c.frontText}] (${c.backText})`).join('\n')
        : "  (لا توجد)";

      const wantsImages = req.body.includeImages !== false; // Enabled by default unless explicitly disabled

      let imageInstruction = "";
      if (wantsImages) {
        imageInstruction = `
6. 🖼️ **إدراج وتوزيع الصور التوضيحية البصرية لتثبيت المعلومات (وضع الصور دائم وتفاعلي 🎨)**:
   - **عامل عنصر الصورة كأداة شرح أساسية وطبيعية حالها كحال الكلمات والنصوص** لترسيخ المفردات في ذهن المتعلم.
   - **قاعدة الكثافة والتثبيت البصري (مهم جداً)**:
     * **مع كل نقطة أو مثال فرعي أو مقارنة**: أرفق عنصر صورة مناسب \`$$IMAGE:{...}$$\` مع كل نقطة أو مثال تشرحه مباشرة (نقطة الشرح ⬅️ صورة توضيحية تثبت المعلومة ⬅️ النقطة التالية مع صورتها وهكذا).
     * **للملاحظات المهمة أو الكلمات المفتاحية**: أرفق صورة بعد الملاحظة أو المفهوم لتجسيده بصرياً للمتعلم.
     * **المعارض أو المقارنات**: يمكنك أيضاً استخدام المعرض التجميعي \`$$IMAGES:[...]\` عند الحاجة للمقارنة بين عدة أشياء.
     * **مع جميع الأوضاع (مختصر، متوازن، مفصل، بقالب أو بدون قالب)**: لا تبخل بالصور، وضع دائماً صوراً متعددة دقيقة لكل نقطة تشرحها في ردك.
   - **صيغ الإدراج**:
     - صورة مفردة (بينية مع أي نقطة أو فقرة):
       \`$$IMAGE:{"query":"clear concrete english search terms","caption":"شرح موجز","size":"medium","keyword":"الكلمة المستهدفة"}$$\`
     - مجموعة صور:
       \`$$IMAGES:[{"query":"term 1","caption":"شرح 1","size":"small","keyword":"word 1"},{"query":"term 2","caption":"شرح 2","size":"small","keyword":"word 2"}]$$\`
   - **قواعد الأحجام**:
     - \`"medium"\`: الحجم المثالي والأكثر طبيعية ليكون مدمجاً بين أسطر الشرح ومع النقاط.
     - \`"small"\`: ممتاز للصور المصغرة المرافقة للأمثلة والملاحظات السريعة.
     - \`"large"\`: للمفهوم الرئيسي البارز.`;
      } else {
        imageInstruction = `
6. 🖼️ **الدعم البصري الطبيعي لترسيخ المعلومات (Smart Visual Elements)**:
   - عامل وسوم الصور كأداة تعبير طبيعية حالها حال الكلمات، ولك كامل الحرية في إدراج عنصر صورة بيني \`$$IMAGE:{...}$$\` مع أي نقطة أساسية أو فرعية تحتاج تثبيتاً بصرياً.
   - الصيغة:
     \`$$IMAGE:{"query":"clear english search terms","caption":"شرح موجز","size":"medium","keyword":"الكلمة المستهدفة"}$$\``;
      }

      // Structure guideline based on user setting
      let structureDirective = "";
      if (useStructuredTemplate && responseLength !== "concise") {
        structureDirective = `
2. **التنسيق الهيكلي (Structured Markdown)**:
   - قسّم الرد إلى أقسام وعناوين فرعية منظمة بالأرقام مثل: \`### 1. **المعنى والاستخدام**\` أو \`### 2. **أداة التعريف والجمع**\`
   - استخدم الفواصل الأفقية \`---\` بين الأقسام الرئيسية لترتيب القراءة.
   - استخدم القوائم النقطية (\`*\` أو \`-\`) والقوائم المرقمة (\`1.\`، \`2.\`) مع إزاحات وتفريعات واضحة للأمثلة وحالات الإعراب.
   - استخدم جداول Markdown (\`| العمود 1 | العمود 2 |\`) لتنظيم تصريفات الأفعال أو مقارنة المرادفات عند الحاجة.
   - استخدم الخط العريض \`**نص عريض**\` للمصطلحات الهامة والملاحظات.`;
      } else {
        structureDirective = `
2. **الأسلوب التفاعلي المباشر (Free Natural Form - بدون قالب إجباري)**:
   - أجب بشكل طبيعي وسلس ومباشر دون التقيّد بهيكل عناوين أو جداول أو تقسيمات إجبارية.
   - ركز فقط على ما طلبه المستخدم تحديداً.`;
      }

      const isChallengeMode = Boolean(
        req.body.challengeContext?.isChallengeMode ||
        card?.isSpokenChallenge ||
        (folderInfo?.name && folderInfo.name.includes("تحدي التحدث"))
      );

      // Format surrounding environment system prompt
      const systemInstruction = isChallengeMode ? `أنت "المصحح والمساعد اللغوي الذكي المتقدم" المتخصص في وضع تحدي التحدث والنطق التفاعلي (Interactive AI Spoken German Challenge Tutor).
الطالب يتدرب حالياً على التحدث والنطق وصياغة الجمل باللغة الألمانية (${frontLang}) بناءً على سيناريوهات ومواقف واقعية.

🎯 **الجملة الألمانية الحالية المستهدفة بالمناقشة والشرح (Current Target Sentence)**:
- الجملة الألمانية المستهدفة: "${cardFront}"
- المطلوب أو المعنى بالعربية: "${cardBack}"
- الموقف أو السياق أو التركيز النحوي: "${cardHint || 'لا يوجد'}"
${card?.grammarFocus ? `- التركيز النحوي المحدد: "${card.grammarFocus}"` : ""}
${card?.cefrLevel ? `- المستوى اللغوي: "${card.cefrLevel}"` : ""}

⏮️ **الجمل السابقة في التحدي (السوابق - للاطلاع وفهم تسلسل التحديات والمواقف)**:
${prevContext}

⏭️ **الجمل القادمة في التحدي (الجايين - للاطلاع فقط لفهم مسار التمرين دون استباقها أو حرقها)**:
${nextContext}

🛑 **قاعدة التركيز الحصري**:
- تركيزك الكامل والإلزامي هو على **الجملة الحالية فقط ("${cardFront}")** وسياقها واستفسارات الطالب حولها.
- ساعد الطالب في فهم تركيب الجملة، تصريف الأفعال، قواعدها النحوية، مخارج حروفها ونطقها الطبيعي، وأي بدائل صياغة يومية أخرى تناسب الموقف.
- لا تذكر الجمل القادمة ولا تحرقها في ردك.

✨ **تعليمات أسلوب الرد والتنسيق المعتمدة**:
1. **طول الإجابة وتنسيقها**:
${lengthDirective}
${structureDirective}
3. **اقتباس المفردات والكلمات (حاسم جداً)**:
   - عندما تذكر أو تقتبس أي كلمة أو جملة باللغة الأجنبية المستهدفة (${frontLang})، ضعها بين علامتي تنصيص \`"word"\` أو \`«word»\` حتى تظهر للمستخدم ككلمة تفاعلية يمكنه النقر عليها للاستماع أو النسخ أو تحويلها لبطاقة فلاش كارد فوراً.
4. **تنبيه هام جداً**:
   - يمنع منعاً باتاً كتابة أي وسوم HTML مثل <br> أو <p>. استخدم فقط أسطر \`\\n\` الطبيعية وقوائم Markdown.
   - استخدم الرموز التعبيرية (Emojis) بشكل متوازن ولطيف لتنشيط الشرح.
5. **التصحيح الذكي لجمل الطالب والتحدث (Smart Correction)**:
   - إذا كتب أو نطق المستخدم جملة وسأل عن صحتها، أو حاول التعبير بأسلوبه، وضح له هل جملته صحيحة ومقبولة عند الألمان؟ ثم قدم النسخة المصححة مع الشرح النحوي المبسط واقتراح أسلوب ناطق أصلي (Native).${imageInstruction}`
      : `أنت "المصحح والمساعد اللغوي الذكي المتقدم" داخل جلسة مراجعة الفلاش كارد (Interactive AI Flashcard Tutor & Corrector).
أنت متواجد حالياً مع الطالب أثناء مراجعته للبطاقات التعليمية، ومهمتك تقديم شرح ذكي، تصحيح دقيق لجمل المستخدم، توضيح الفروقات الدلالية والقواعد وأدوات التعريف، والإجابة عن أسئلته بدقة.

🎯 **البيئة والسياق التعليمي الشامل للجلسة (Context Awareness)**:
- 📁 **المجلد الحالي**: "${folderTitle}"
- 📝 **وصف المجلد**: "${folderDesc}"
- 🌐 **اللغة المستهدفة للبطاقة**: "${frontLang}"
- 🌐 **لغة الترجمة / الشرح**: "${backLang}"

🃏 **البطاقة الحالية المستهدفة بالمناقشة والشرح (Focus Target)**:
- الوجه الأمامي (الكلمة/الجملة): "${cardFront}"
- أداة التعريف (إن وجدت): "${cardArticle || 'غير محددة'}"
- صيغة الجمع (إن وجدت): "${cardPlural || 'غير محددة'}"
- الوجه الخلفي (الترجمة/المعنى): "${cardBack}"
- التلميح والوصف: "${cardHint || 'لا يوجد'}"

🛑 **قاعدة التركيز الحصري الصارمة (Strict Current Card Focus - منع الانتقال أو الاقتراح)**:
- تركيزك الكامل والإلزامي هو على **البطاقة الحالية فقط ("${cardFront}")**.
- ❌ **يُمنع منعاً باتاً**: اقتراح الانتقال للبطاقة التالية، أو ذكر أو شرح أي بطاقة من البطاقات التالية، أو كتابة عبارات مثل "هل ننتقل للبطاقة التالية؟" أو "البطاقة القادمة هي...".
- البطاقات السابقة والتالية المذكورة أدناه هي **فقط لفهم السياق العام ومستوى المجلد** ولا يجوز إطلاقاً ذكرها أو اقتراح الانتقال إليها في ردك!

📚 **سياق خلفي عام للمجلد (للاطلاع وفهم موضوع الملف فقط دون ذكرها للمستخدم)**:
- بطاقات أخرى في الملف:
${prevContext}
${nextContext}
...
${structureDirective}
3. **اقتباس المفردات والكلمات (حاسم جداً)**:
   - عندما تذكر أو تقتبس أي كلمة أو جملة باللغة الأجنبية المستهدفة (${frontLang}) مثل "die Speisekarte" أو "Speisekarten" أو "sehen"، ضعها بين علامتي تنصيص \`"word"\` أو \`«word»\` حتى تظهر للمستخدم ككلمة تفاعلية يمكنه النقر عليها للاستماع أو النسخ أو تحويلها لبطاقة فلاش كارد فوراً.
4. **تنبيه هام جداً**:
   - يمنع منعاً باتاً كتابة أي وسوم HTML مثل <br> أو <p>. استخدم فقط أسطر \`\\n\` الطبيعية وقوائم Markdown.
   - استخدم الرموز التعبيرية (Emojis) بشكل متوازن ولطيف لتنشيط الشرح.
5. **التصحيح الذكي (Smart Correction)**:
   - إذا كتب المستخدم جملة وسأل عن صحتها، وضح صحتها أولاً ثم قدم النسخة المصححة مع الشرح النحوي المبسط واقتراح أسلوب ناطق أصلي (Native).${imageInstruction}`;

      // Build conversation history for multi-turn (up to 30 messages)
      const limitedHistory = (Array.isArray(chatHistory) ? chatHistory : []).slice(-30);

      // Construct current turn user prompt with runtime override directive
      let currentTurnPrompt = message;
      const runtimeDirectives: string[] = [];

      if (responseLength === "concise") {
        runtimeDirectives.push(`⚡ أجب بأسلوب "موجز ومركّز": ركّز على زبدة الفائدة اللغوية وإجابة السؤال مباشرة بأقصر عبارة كافية ووافية دون مقدمات أو حشو ودون محاكاة طول الردود السابقة.`);
      } else if (responseLength === "auto") {
        runtimeDirectives.push(`🎯 أسلوب "تلقائي ذكي": حدد طول وعمق الرد بناءً على طبيعة السؤال (إذا كان بسيطاً أو مباشراً أجب بإيجاز سريع في سطرين، وإذا كان تحليلياً أو مقارنة اشرحه بتفصيل كافٍ).`);
      } else if (responseLength === "detailed") {
        runtimeDirectives.push(`📚 أجب بأسلوب "مفصل وشامل": يرجى تقديم شرح ثري وتفصيلي مع أمثلة وسياقات غنية حتى لو كانت الردود السابقة قصيرة.`);
      }

      if (wantsImages) {
        runtimeDirectives.push(`🖼️ وضع الصور مفعّل وتفاعلي: عامل عناصر الصور $$IMAGE:{...}$$ كأداة شرح طبيعية وأساسية. أرفق صورة توضيحية دقيقة مع كل نقطة أو مثال أو ملاحظة لترسيخ وتثبيت المعنى بصرياً دون تردد.`);
      } else {
        runtimeDirectives.push(`🖼️ وضع الصور التلقائي غير مفروض: لك كامل الحرية في إدراج عنصر صورة بيني مع أي نقطة أو مثال إذا كان يثبت المعنى اللغوي ويرسخه.`);
      }

      if (runtimeDirectives.length > 0) {
        runtimeDirectives.push(`🛑 ركّز حصراً على البطاقة الحالية "${cardFront}". يُمنع نهائياً اقتراح الانتقال لبطاقة أخرى أو ذكر بطاقات أخرى.`);
        currentTurnPrompt = `${message}\n\n[🚨 توجيهات فورية ملزمة لهذا الرد: ${runtimeDirectives.join(" | ")}]`;
      } else {
        currentTurnPrompt = `${message}\n\n[🚨 ركّز حصراً على البطاقة الحالية "${cardFront}". يُمنع نهائياً اقتراح الانتقال لبطاقة أخرى أو ذكر بطاقات أخرى]`;
      }
      ...
      if (!responseText) {
        ...
        if (!aiClient) {
          if (effectiveGroqKey && !isGroq) {
            // Groq fallback if gemini key is missing
            try {
              const groqMessages = [
                { role: "system", content: systemInstruction },
                ...limitedHistory.map((m: any) => ({
        ...
        if (!responseText && aiClient) {
          ...
          if (primaryModel.includes("groq")) primaryModel = "gemini-3.6-flash";
          if (primaryModel.includes("grok")) primaryModel = "gemini-3.6-flash";
          ...
          for (const modelToTry of candidateModels) {
            try {
              ...
              console.warn(`[Review Chat] Model '${modelToTry}' failed (${errMsg}). Trying next candidate...`);
      ...
      if (!responseText) {
        return res.status(500).json({
          error: "تعذر توليد رد من نماذج الذكاء الاصطناعي بسبب الضغط العالي على الخوادم. يرجى المحاولة بعد لحظات.",
          warnings: modelWarnings
        });
      }
      ...
      console.error("AI Review Chat error:", err);
